...
time_period_to_save = ["Pre-surgery","Post-surgery","2 weeks","4 weeks"]
time_period = ["pre","post","14 days","1 month"]
#time_period = ["2 weeks"]
...
for p,l in zip(force_files,knee):
    ...
    for t,ts in zip(time_period,time_period_to_save):
        ...
        found = False
        for file in result:
            if file[:3] == 'sta' or file[:3] == 'bip':
                total_path = f'{path_in}\{file}'
        ...
        os.chdir('F:\Kinvent\Osteoarthitis\Scripts\cop_data')
        Excel_Both_legs_df.to_excel(writer,sheet_name=ts)
